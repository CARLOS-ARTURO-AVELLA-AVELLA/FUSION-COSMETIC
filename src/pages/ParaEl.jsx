...
export default function ParaEl() {
  const navigate = useNavigate()
  const { products, filteredProducts, filterProducts, sortProducts, searchProducts } = useProductStore()
  const [sortType, setSortType] = useState('popular')
  const [searchQuery, setSearchQuery] = useState('')
  
  // Calcular el precio máximo dinámicamente para productos de hombre
  const menProducts = products.filter(p => p.genero === 'hombre')
  const maxPrice = menProducts.length > 0 ? Math.max(...menProducts.map(p => p.precio)) : 1000000
  
  const [priceRange, setPriceRange] = useState({ min: 0, max: maxPrice })

  // Filtrar productos para hombres al cargar
  useEffect(() => {
    filterProducts({ 
      genero: 'hombre',
      ...
      <section className="bg-gradient-to-r from-turquoise to-gold py-16 border-b-2 border-turquoise/20">
        <div className="max-w-7xl mx-auto px-4">
          <h1 className="text-5xl font-bold font-display text-white mb-3">
            Para <span className="font-light">Él</span>
          </h1>
          <p className="text-white/90 text-lg max-w-2xl">
            Descubre nuestra colección exclusiva de fragancias para hombres. Desde aromas frescos y cítricos 
            hasta sofisticadas fragancias amaderadas, encuentra el perfume que refleje tu estilo.
          </p>
        </div>
      </section>
        ...
        <div className="max-w-7xl mx-auto px-4">
          ...
          {filteredProducts.length > 0 ? (
            ...
            <div className="text-center py-16">
              ...
              >
                Ver Todos los Productos para Él
              </button>
            </div>
          )}
        </div>
      ...
      <section className="py-16 bg-gradient-to-r from-turquoise to-gold">
        <div className="max-w-7xl mx-auto px-4 text-center">
          ...
          <p className="text-white/90 text-lg mb-8 max-w-2xl mx-auto">
            ¿Eres perfumista o dueño de una perfumería? Contáctanos para que tus nuevos lanzamientos 
